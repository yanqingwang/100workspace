...
class ap_factor_data(object):

    def __init__(self):
        self.country = {"AU":"AUS","CN":"CHN",'ID':'IDN','JP':'JPN','KR':"KOR",'MY':'MYS',
                        'PH':'PHL','SG':'SGP','TW':'TWN','TH':'THA','AE':'ARE','VN':'VNM'}
        self.root = 'c:/temp/Quality/'
        self.empfile = 'AP_EmployeeBasicInfov2light-20200229.xlsx'
        self.bcs_data = 'DataQualityReport_02_2020_with_country.xlsm'
    ...
    def check_email(self,email):
        ...
        try:
            if re.match(r'^([a-zA-Z0-9.-]+)@zf.com$',lv_mail) or re.match(r'^([a-zA-Z0-9.-]+)@trw.com$',lv_mail)\
                    or re.match(r'^([a-zA-Z0-9.-]+)@zf[0-9a-zA-Z-]{0,13}\.com$',lv_mail)\
                    or re.match(r'^([a-zA-Z0-9.-]+)@aac.co.th$',lv_mail)\
                    or re.match(r'^([a-zA-Z0-9.-]+)@fmg[0-9a-zA-Z-]{0,13}\.com$',lv_mail)\
                    :
                return 1
            else:
                return 0
    ...
    def get_emp_data(self):
        ...
        try:
            df_raw = pd.DataFrame(pd.read_excel(io=emp_file, sheet_name='Excel Output', dtype = {'ZF Global ID':str},header=0, skiprows=2))
            # print(df.columns)
    ...
    def quality_check_summary(self):
        ...
        file_out = 'Output_' + now_date + '_ap_summary.xlsx'
        ...
        df_simple = df_data.groupby(['Country (ID)', 'Company (Label)', 'RU'])[
            ...
            'BP_Chck','DATE_CHK','EMAIL_ZID_CHK','ID_email_check'].sum().reset_index()
        ...
        df_RU_res = df_res.groupby(['Country', 'Country (ID)', 'Company (Label)','DIV' , 'RU' ])['Total_EMP'].sum().reset_index()
        df_RU_res.sort_values(by = ['Country','Country (ID)','Company (Label)','DIV',"Total_EMP"], ascending=[True,True,True,True,False],inplace=True)

        df_headcount = df_res.groupby(['Country', 'Country (ID)','DIV','RU'])['BCS_EMPs','SF_EMPs','GAPS_EMP','ABS_GAPS_EMP'].sum().reset_index()
        df_headcount.sort_values(by = ['Country (ID)','DIV',"ABS_GAPS_EMP"], ascending=[True,True,False], inplace=True)

        df_emails = df_data[['ZF Global ID', 'ZID', 'Employee Status (Label)','First Name','Last Name','RU','DIV','Business email address','Country (ID)',
                             'email_check','ZID_check','EMAIL_ZID_CHK']]
        df_emails = df_emails.loc[(df_emails['EMAIL_ZID_CHK'] > 0) ]

        df_mgr = df_data[['ZF Global ID', 'ZID', 'Employee Status (Label)','First Name','Last Name','RU','DIV','Business email address','Country (ID)',
        ...
        try:
            ...
            {'font_size': 10, 'align': 'right', 'valign': 'vcenter', 'text_wrap': True,'num_format': "0.0%"})
            one_format = workbook.add_format(
            {'font_size': 10, 'align': 'right', 'valign': 'vcenter', 'text_wrap': True,'num_format': "0%"})
            # chart2 = workbook.add_chart({'type': 'column'})        #'subtype': 'percent_stacked'
            ...
            self.out_mails(df_data, df_writer, workbook,sheet_name,one_format)
            sheet_name = '212_Email_CHN'
            self.out_mails(df_res_chn, df_writer, workbook,sheet_name,one_format)

            sheet_name = '220_Email_ZID'
            self.chk_zid_mail(df_res, df_writer, workbook,sheet_name)

            sheet_name = '230_MGR_AP'
            self.chk_mgr(df_res, df_writer, workbook,sheet_name,one_format)
            sheet_name = '232_MGR_CHN'
            self.chk_mgr(df_res_chn, df_writer, workbook,sheet_name,one_format)

            sheet_name = '240_DATE_AP'
            self.check_date_match(df_res, df_writer, workbook,sheet_name,common_format)
            sheet_name = '242_DATE_CHN'
            self.check_date_match(df_res_chn, df_writer, workbook,sheet_name,common_format)
