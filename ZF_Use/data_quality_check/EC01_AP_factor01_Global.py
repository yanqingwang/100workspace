...
class ApFactorData(object):
    def __init__(self):
        ...
        self.empfile = 'EmployeeBasicInfov2all-Page1-20191231.xlsx'
    ...
    def get_emp_data(self):
        ...
        try:
            df_raw = pd.DataFrame(pd.read_excel(io=emp_file, sheet_name='Excel Output', dtype = {'ZF Global ID':str,'Admin Group (Name)':str},header=0, skiprows=2))
            # print(df.columns)
            df = df_raw[['ZF Global ID','ZID',	'Local ID',	'Employee Status (Label)',	'First Name',	'Last Name',	'Alternate First Name',
                    'Alternate Last Name',	'Date Of Birth',	'Hire Date'	,'Original Start Date',	'Termination Date'	,
                    'Solid Line Manager Global ID',	'Solid Line Manager (Last Name)',	'Solid Line Manager Position',	'Solid Line Manager (First Name)',
            ...
            df['AgeLess18'] = df.apply(lambda x: 1 if (x['AGE_RANGE'] == '0--18' or x['AGE_RANGE'] == 'Unkown') else 0, axis=1)
    ...
    def out_raw_data(self,folder,field,range,df_headcount,df_emails,df_mgr,df_age18,df_entry_date):
    ...
    def quality_check_summary(self):
        ...
        list_admin = list(set(df_data['Admin Group (Name)'].tolist()))

        # All
        df_simple = df_data.groupby(['Country (ID)', 'Company (Label)', 'RU'])[
            'Total_EMP','Intern', 'Apprentices', 'VacationWorker', 'email_check','ZID_check','SM_Chck','IM_Chck',
            'BP_Chck','DATE_CHK','EMAIL_ZID_CHK','ID_email_check','AgeLess18'].sum().reset_index()
        ...
        df_res2 = df_res.groupby(['Country', 'Country (ID)', 'DIV' , 'RU','Admin Group (Name)' ])[
            'Total_EMP','email_check','ZID_check','SM_Chck','IM_Chck','BP_Chck','ID_email_check',
            'SM_GAP','IM_GAP','BP_GAP', 'AgeLess18',
             'DATE_CHK','EMAIL_ZID_CHK','BCS_EMPs','SF_EMPs','BCS_Others','SF_Others',
        ...
        df_headcount['HD_GAP_RATE'] = df_headcount.apply(lambda x: round(x['ABS_GAPS_EMP'] / x['BCS_EMPs'],3), axis=1)
        df_headcount.sort_values(by = ["ABS_GAPS_EMP"], ascending=[False], inplace=True)

        df_emails = df_data[['Country (ID)','Company (Label)','DIV','RU','Admin Group (Name)','ZF Global ID', 'ZID', 'Employee Status (Label)','First Name','Last Name',
                             'Business email address', 'email_check','ZID_check','EMAIL_ZID_CHK']]
        df_emails = df_emails.loc[(df_emails['EMAIL_ZID_CHK'] > 0) ]

        df_mgr = df_data[['Country (ID)','Company (Label)','DIV','RU','Admin Group (Name)','ZF Global ID', 'ZID', 'Employee Status (Label)','First Name','Last Name',
        ...
        df_entry_date = df_data[['Country (ID)','Company (Label)','DIV','RU','Admin Group (Name)','ZF Global ID', 'ZID', 'Employee Status (Label)','First Name','Last Name',
                            'Hire Date', 'Original Start Date','DATE_CHK']]
        df_entry_date = df_entry_date.loc[(df_entry_date['DATE_CHK'] > 0) ]

        df_age18 = df_data[['Country (ID)','Company (Label)','DIV','RU','Admin Group (Name)','ZF Global ID', 'ZID', 'Employee Status (Label)','First Name','Last Name',
        ...
        try:
            ...
            self.out_mails(df_data, df_writer, workbook,sheet_name,self.one_format)
            sheet_name = '212_Email_CHN'
            self.out_mails(df_res_chn, df_writer, workbook,sheet_name,self.one_format)
    ...
    #     RAW Data By Division
        ...
        self.out_raw_data('/DIV/','DIV',self.division,df_headcount,df_emails,df_mgr,df_age18,df_entry_date)
        ...
        self.out_raw_data('/LE/','Company (Label)',list_cc,df_headcount,df_emails,df_mgr,df_age18,df_entry_date)

        #     RAW Data By Admin Group
        # print('Prepare RAW Data by Admin Group ----------------------------------------------------')
        # self.out_raw_data('/Admin/','Admin Group (Name)',list_admin,df_headcount,df_emails,df_mgr,df_age18,df_entry_date)
